...
class VideoElement extends HTMLElement {
  constructor() {
    ...
    if (this.autoplay) {
      // Disable scroll animations - play immediately
      if (!this.paused) {
        this.play();
      }
    }
...
class GridList extends HTMLElement {
  constructor() {
    ...
    // Disable scroll animations - show items immediately
    this.showGridItemsImmediately();
  }
  ...
  showGridItemsImmediately() {
    // Show all grid items immediately without animation
...
class AnnouncementBar extends HTMLElement {
  constructor() {
    ...
    // Disable scroll animations - initialize carousel immediately
...
class MotionElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this.isInstant && this.media) {
      FoxTheme.utils.imageReady(this.media).then(() => {
        this.initialize();
      });
    } else {
      this.initialize();
...
document.addEventListener('click', function (event) {
  console.log('run1')
  const openButton = event.target.closest('[data-open-sort]');
  if (openButton) {
    const drawer =
      openButton.parentElement?.querySelector('#SortDrawer') ||
      openButton.closest('#SortByContainer')?.querySelector('#SortDrawer') ||
      document.getElementById('SortDrawer');
    console.log('run2')
    if (drawer) {
  ...
  const closeTarget = event.target.closest('.sort-drawer__overlay,[data-close-sort]');
  if (closeTarget) {
    const drawer = closeTarget.closest('.sort-drawer') || document.querySelector('.sort-drawer.active');
    if (drawer) {
