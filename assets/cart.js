...
class CartItems extends HTMLElement {
  ...
  validateQuantity(event) {
    ...
    let message = '';
    if(target.name == 'variant-dropdown' && window.location.href.includes('/cart')) {
      index = parseInt(target.dataset.line);
      inputValue = 0
      let varinatId = target.value;
      target = document.getElementById('CartItem-'+index).querySelector('.quantity__input');
      console.log("1", inputValue)
      console.log("here 22", varinatId)
      fetch('/cart/add.js', {
          method: 'POST',
          headers: { 
      ...
      .then(response => {
        ...
        if (!response.ok) {
          ...
          this.updateQuantity(plusIndex, 0, document.activeElement.getAttribute('name'), target);
          return false;
        }
        return response.json();
      })
      return; // Added return to prevent falling through to quantity validation
    }
    if (inputValue < parseInt(target.getAttribute('data-min'))) {
      ...
      message = FoxTheme.quickOrderListStrings.max_error.replacupdateQuantitye('[max]', target.max);
  ...
  onCartUpdate(event) {
    ...
    //restore coupon button states after DOM re-render
    setTimeout(() => {
      if (window.restoreDiscountButtonStates) {
        window.restoreDiscountButtonStates();
      }
    }, 100);
  }
  ...
  updateQuantity(line, quantity, name, target) {
    ...
    const currentSectionId = FoxTheme.utils.getSectionId(this);
    if (currentSectionId && !sectionsToBundle.includes(currentSectionId)) {
      sectionsToBundle.push(currentSectionId);
    }
